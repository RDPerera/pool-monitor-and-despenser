...
class DeviceProvider with ChangeNotifier {
  ...
  Future<void> loadDevices() async {
    ...
    try {
      ...
      if (_devices.isEmpty && kDebugMode) {
        ...
        // Pick the device with the most recent last_seen
        _devices.sort((a, b) {
          final aTime = DateTime.tryParse(a.lastSeen ?? '') ?? DateTime(0);
          final bTime = DateTime.tryParse(b.lastSeen ?? '') ?? DateTime(0);
          return bTime.compareTo(aTime);
        });
        _selectedDevice = _devices.first;
        await loadLatestReading(_selectedDevice!.deviceId);
        await loadDeviceReadings(_selectedDevice!.deviceId, limit: 48);
      }
    } catch (e) {
  ...
  Future<void> selectDevice(String deviceId) async {
    _selectedDevice =
        _devices.firstWhere((device) => device.deviceId == deviceId);
    await loadLatestReading(deviceId);
    notifyListeners();
  }

  Future<void> loadDeviceReadings(String deviceId,
      {int limit = 100, int? hours}) async {
    _setLoading(true);
    _setError(null);
  ...
  Future<void> loadLatestReading(String deviceId) async {
    try {
      debugPrint(
          '[DeviceProvider] Fetching latest reading for device: $deviceId');
  ...
  Future<bool> updateDeviceConfig(String deviceId, DeviceConfig config) async {
    ...
    try {
      if (_deviceConfig == null) {
        _deviceConfig =
            await _deviceService.createDeviceConfig(deviceId, config);
      } else {
        _deviceConfig =
            await _deviceService.updateDeviceConfig(deviceId, config);
      }
      return true;
    } catch (e) {
      _setError(e.toString().replaceFirst('Exception: ', ''));
  ...
  Future<void> loadDeviceStats(String deviceId, {int hours = 24}) async {
    ...
    try {
      _deviceStats =
          await _deviceService.getDeviceStats(deviceId, hours: hours);
    } catch (e) {
      _setError(e.toString().replaceFirst('Exception: ', ''));
    } finally {
      _setLoading(false);
  ...
  Future<bool> updateDevice(String deviceId,
  ...
  Future<bool> emergencyStop(String deviceId) async {
    _setLoading(true);
    _setError(null);

    try {
      await _deviceService.emergencyStop(deviceId);
      return true;
    } catch (e) {
      _setError(e.toString().replaceFirst('Exception: ', ''));
      return false;
  ...
  String getChlorineStatus() {
    ...
    if (_latestReading!.chlorineStatus != null) {
      return _latestReading!.chlorineStatus!;
    }
    final estimated = getEstimatedChlorine();
    if (estimated < 1.2) return 'LOW';
    if (estimated < 2.5) return 'NORMAL';
    return 'HIGH';
  }

  int getRecommendedChlorineDose() {
    if (_latestReading == null) return 0;
